...
To reproduce the .scan files, add the --parameter-scan option to produce XXX_scan.dat. This file starts with a three-line header that describes the baseline fit, followed by one line per scan point. The scan point lines give each parameter value (including fixed parameters) in the same order that they are normally displayed, followed by the chisq value. The anisotropic BAO scale parameters, alpha-parallel and alpha-perp, are columns 7 and 8 (auto-correlation) or 10 and 11 (cross-correlation) counting from 0.
