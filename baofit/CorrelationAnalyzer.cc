...
void local::CorrelationAnalyzer::generateMarkovChain(int nchain, const std::string &mcmcConfig, int interval,
    ...
    likely::FitParameters parametersC(fminCopy->getFitParameters());
    std::cout << "modifying parameters >" <<mcmcConfig<<"|"<<std::endl;
    likely::modifyFitParameters(parametersC,mcmcConfig);
    fminCopy->updateParameters(fminCopy->getMinValue(), parametersC);
    printFitParametersToStream(fminCopy->getFitParameters(),std::cout);
    std::cout << "done modifying parameters" <<std::endl;
