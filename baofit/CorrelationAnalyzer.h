...
namespace baofit {
	...
	public:
        ...
        int fitEach(likely::FunctionMinimumPtr fmin,
            likely::FunctionMinimumPtr fmin2 = likely::FunctionMinimumPtr(),
            std::string const &refitConfig = "", std::string const &saveName = "", int nsave = 0) const;
        // Refits on the parameter grid specified by each parameter's binning spec and save the
        // results to the specified file name. Returns the number of fits performed.
        int parameterScan(std::string const &saveName = "") const;
