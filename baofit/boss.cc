...
baofit::AbsCorrelationDataPtr local::createDR9LRGPrototype(double zref,
...
local::loadDR9LRG(std::string const &dataName, baofit::AbsCorrelationDataCPtr prototype, bool verbose) {
    ...
    while(std::getline(paramsIn,line)) {
        ...
        bin[1] = 0;
        try {
            int monoIndex = grid.getIndex(bin);
...
// Creates a prototype MultipoleCorrelationData with the specified binning.
baofit::AbsCorrelationDataPtr local::createFrenchPrototype(double zref) {
    // Create the new BinnedData that we will fill.
    likely::AbsBinningCPtr
...
bool verbose, bool unweighted, bool expanded) {
    ...
    while(std::getline(paramsIn,line)) {
        ...
        int quadIndex = grid.getIndex(bin);
    ...
    if(!unweighted) {
        ...
        while(std::getline(covIn,line)) {
            ...
            binnedData->setCovariance(grid.getIndex(bin1), grid.getIndex(bin2), cov);
        }
        covIn.close();
        if(verbose) {
...
baofit::AbsCorrelationDataPtr local::createCosmolibXiPrototype(double minz, double dz, int nz,
